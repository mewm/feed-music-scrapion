...
Scrapion.prototype.processMessage = function(msg, cb) {
			...
			if(inCache != null) {
				...
				stream.on('end', function() {
					...
					var timeTaken = ((now() - startTime) / 1000).toFixed(1),
						cleanFilename = metadata.title.replace(/\s/g, '_')  + '.mp3',
	...
	if(config.feedSource == 'HipChat') {
		...
		setInterval(function() {
			...
			self.feeder.getMessages(function(messages) {
				...
				for(message in messages) {
					...
					self.processMessage(msg, function() {
						//Done
					})
		...
		self.feeder.bot.on('message', function (from, to, message) {
			if(to == '#misuc') {
				...
				//First first word
				var splitCommand = message.split(' ');
				var command = splitCommand[0];
				var msg = splitCommand[1];
				...
				switch (command) {
					...
					case '!vote':
						...
						if(!(msg in scoreBoard)) {
							scoreBoard[msg] = 1;
						}  else {
							scoreBoard[msg] += 1;
						}

						if(scoreBoard[msg] == voteSuccess) {
							self.feeder.notify('Vote count matched with ' + voteSuccess + ' | URL: ' + msg);
							self.processMessage(msg, function() {
								scoreBoard[msg] = 0;
							})
						}
				...
				console.log(scoreBoard[msg] + ': votes on ' + msg);
